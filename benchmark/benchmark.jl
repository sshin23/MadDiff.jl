...
for (N,nlm,jump,name) in [
    ...
    for i=1:1
        GC.enable(false)
        m = jump(;N=N,optimizer=() -> AmplNLWriter.Optimizer(Ipopt_jll.amplexe))
        ...
        GC.enable(false)
        t1 = @elapsed begin
            m = nlm(;N=N,optimizer=Ipopt.Optimizer,
                    output_file="output/$name-simplenlmodels.out",
                    linear_solver="ma27")
            MadDiff.optimize!(m)
        end
        ...
        GC.enable(false)
        t7 = @elapsed begin
            m = jump(;N=N,optimizer=Ipopt.Optimizer)
            set_optimizer_attribute(m,"linear_solver","ma27")
            JuMP.optimize!(m)
        end
