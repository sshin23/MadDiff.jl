for (forig,dorig,points) in [
    ...
    (x->0*x[1], Dict(1=>x->0.), [randn(1) for i=1:3]),
    (x->x[1]+0*x[1], Dict(1=>x->1.), [randn(1) for i=1:3]),
    (x->0*x[1]+x[1], Dict(1=>x->1.), [randn(1) for i=1:3]),
    ...
    (x->x[1]-x[2], Dict(1=>x->1.,2=>x->-1.),[randn(2) for i=1:3]),
    (x->-(x[2]-x[1]), Dict(1=>x->1.,2=>x->-1.),[randn(2) for i=1:3]),
