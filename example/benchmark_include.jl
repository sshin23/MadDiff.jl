function nlm_ocp(;N=1)
    ...
    for i=1:N
        ...
        constraint(
            m, -x[i+1,2] + x[i,2] + (u[i,1]*cos(x[i,7])*sin(x[i,8])*cos(x[i,9])+u[i,1]*sin(x[i,7])*sin(x[i,9]))*dt)
        constraint(
            m, -x[i+1,3] + x[i,3] + (x[i,4])*dt)
        constraint(
            m, -x[i+1,4] + x[i,4] + (u[i,1]*cos(x[i,7])*sin(x[i,8])*sin(x[i,9])-u[i,1]*sin(x[i,7])*cos(x[i,9]))*dt)
        constraint(
            m, -x[i+1,5] + x[i,5] + (x[i,6])*dt)
        constraint(
            m, -x[i+1,6] + x[i,6] + (u[i,1]*cos(x[i,7])*cos(x[i,8])-9.8)*dt)
        constraint(
            m, -x[i+1,7] + x[i,7] + (b*u[i,2]*cos(x[i,7])/cos(x[i,8])+u[i,3]*sin(x[i,7])/cos(x[i,8]))*dt)
        constraint(
            m, -x[i+1,8] + x[i,8] + (-b*u[i,2]*sin(x[i,7])+u[i,3]*cos(x[i,7]))*dt)
        constraint(
            m, -x[i+1,9] + x[i,9] + (b*u[i,2]*cos(x[i,7])*tan(x[i,8])+u[i,3]*sin(x[i,7])*tan(x[i,8])+u[i,4])*dt)
    end
